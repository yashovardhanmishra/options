...
export default function StrategyStats({ payoff, book, spot }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-1 gap-y-0 border-b border-edge bg-panel">
      <Cell label="P&L"><span className={pnlCls(pnl)}>{money(pnl)}</span></Cell>
      ...
      <Cell label={credit >= 0 ? 'Net Credit' : 'Net Debit'}>
      ...
      {payoff?.atmIv != null && (
        <Cell label="ATM IV"><span className="text-slate-300">{(payoff.atmIv * 100).toFixed(1)}%</span></Cell>
      )}
    </div>
  )
}
